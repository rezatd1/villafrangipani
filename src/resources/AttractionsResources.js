const AttractionsResources = [
    {
        id: 1,
        info: [{
            id: 1,
            img: '/assets/img/location/attractions/villa_frangipani_canggu_ubud_monkey_forrest.jpg',
            title: 'Ubud and the Sacred Monkey Forest',
            text: 'The Sacred Monkey Forest Sanctuary is a must-visit destination in Bali, just a short drive from Villa Frangipani. This natural forest reserve is home to over 700 long-tailed macaques and features ancient temples and serene walking paths. '
        },
        {
            id: 2,
            img: '/assets/img/location/attractions/villa_frangipani_canggu_ulun_danu_beratan_temple.jpg',
            title: 'Ulun Danu Beratan Temple',
            text: 'Ulun Danu Beratan Temple is a beautiful water temple located in Bali, Indonesia. It is one of the most popular tourist destinations in Bali due to its stunning location on the shores of Lake Beratan. The temple is dedicated to the goddess of water, Dewi Danu, and is surrounded by lush tropical gardens. '
        },
        {
            id: 3,
            img: '/assets/img/location/attractions/villa_frangipani_canggu_tanah_lot_temple.jpg',
            title: 'Tanah Lot',
            text: 'Tanah Lot is a unique rock formation that stands tall just off the coast of Bali, Indonesia. It is home to a Hindu temple that dates back to the 16th century and is a significant religious site for the Balinese people. Visitors come from all over the world to witness the stunning views of the temple perched on the rock, especially during sunset when the temple is bathed in golden light.'
        },
        {
            id: 4,
            img: '/assets/img/location/attractions/villa_frangipani_Pura_Lempuyang_temple_aka_+gates_+of_+heaven_balli.jpg',
            title: 'Nusa Penida',
            text: "Nusa Penida is a small island located southeast of Bali, known for its stunning beaches, crystal clear waters, and natural landmarks like Angel's Billabong and Kelingking Beach. It's a popular destination for day trips or longer stays and offers opportunities for snorkeling, diving, and exploring the island's natural beauty. There are several tour companies that offer full-day or half-day tours of the island."
        },]
    },
    {
        id: 2, info: [{
            id: 1,
            img: '/assets/img/location/attractions/villa_frangipani_Pura_Lempuyang_temple_aka_+gates_+of_+heaven_balli.jpg',
            title: 'Lempuyang Temple Pura aka "Gates of Heaven"',
            text: 'Lempuyang Temple, also known as Pura Lempuyang Luhur, is a complex of seven temples located in East Bali, Indonesia. It is a popular destination for tourists who come to see the "Gates of Heaven" and take in the stunning panoramic views of Mount Agung. The temple is also a significant spiritual site for the Balinese people.',
        },
        {
            id: 1,
            img: '/assets/img/location/attractions/villa_frangipani_canggu_surfing_bali.jpg',
            title: 'Surfing',
            text: 'Bali is known for its world-class surfing spots, with waves that cater to both beginners and experienced surfers. Some of the popular surfing spots in Bali include Kuta Beach, Uluwatu, Padang Padang, Canggu, and Bingin Beach. Each spot offers different types of waves and conditions, making Bali a great destination for surfers of all levels.',
        },
        {
            id: 1,
            img: '/assets/img/location/attractions/villa_frangipani_uluwatu_temple.jpg',
            title: 'Uluwatu Temple',
            text: "Uluwatu Temple is a stunning sea temple located on a cliff in South Bali, offering breathtaking sunset views and a unique cultural experience. It is one of Bali's six key temples and is home to a colony of friendly monkeys.",
        },
        {
            id: 1,
            img: '/assets/img/location/attractions/villa_frangipani_changgu_tegallalang,_rice_terraces.jpg',
            title: 'Tegalalang Rice Terrace',
            text: 'Tegalalang Rice Terrace is a popular tourist attraction located in Ubud, Bali. The stunning rice paddies offer beautiful panoramic views and are a must-see for anyone visiting Bali.',
        },
        {
            id: 1,
            img: '/assets/img/location/attractions/villa_frangipani_canggu_mount_batur_trek_sunrise.jpg',
            title: 'Mount Batur',
            text: 'Mount Batur is an active volcano located in the Bangli regency of Bali. Hiking to the summit to watch the sunrise is a popular activity for tourists, offering stunning views of the surrounding mountains and Lake Batur. The trek is moderate in difficulty and usually takes around 2 hours.',
        },
        ]
    }
];
export default AttractionsResources;