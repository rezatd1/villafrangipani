...
export default function BeachclubsCard({ key, containerClassName, img, title, info1, info2, button }) {
    return (
        <div key={key} className={`light-gray-bg my-5 mx-2 ${containerClassName}`}>
            <div className='py-4 py-lg-5 py-md-5 py-sm-5 px-3 px-lg-4 px-md-4 px-sm-3 d-flex justify-content-center align-items-center flex-column'>
                ...
                <button type="button" class="btn bg-light-blue font-size-1-1 text-white py-3 px-5 my-3">{button}</button>
            </div>
        </div>
    )
}
